...
object DataUtils {
    ...
    fun setRecyclerViewMissions(view: View, activity: ViewUtils, idList: ArrayList<String>?, isLandpad: Boolean) {
        val jsonArray = JSONArray()
        if (idList != null) {
            val client = OkHttpClient()
            idList.forEachIndexed { i, id ->
                ...
                client.newCall(request).enqueue(object : Callback {
                    ...
                    override fun onResponse(call: Call, response: Response) {
                        jsonArray.put(response.body()!!.string())
                        if ((i + 1) == idList.size) {
                            LaunchesRequests(null, activity, view, isLandpad).execute(jsonArray)
                        }
                    }
                })
            }
